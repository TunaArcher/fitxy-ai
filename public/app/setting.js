...
function WABListBusinessAccounts() {
  $.ajax({
    ...
    .done(function (res) {
      let $pages = res.data.pages || []; // ตรวจสอบว่า res.data.pages มีค่าหรือไม่
      ...
      if ($pages.length === 0) {
        // ถ้าไม่มีข้อมูลให้แสดงข้อความว่าไม่พบ Business WhatsApp
        $wrapper.html(`
          <div class="alert alert-warning text-center">
            <strong>ไม่พบ Business WhatsApp</strong>
          </div>
        `);
        return;
      }
      ...
      $pages.forEach((page) => {
        let $btnConnect = `
          <button type="button" class="btnConnectToApp btn btn-primary btn-sm px-2" 
        ...
        if (page.status === "connected") {
          $btnConnect = `
            <button type="button" class="btnConnectToApp btn btn-primary btn-sm px-2 disabled" 
              data-platform="WhatsApp" 
              data-page-id="${page.id}" 
              data-page-name="${page.name}">
              เชื่อมต่อแล้ว
